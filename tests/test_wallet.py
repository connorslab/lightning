...
def test_fundpsbt_feerates(node_factory, bitcoind, chainparams, addrtype):
    ...
    if addrtype == 'bech32':
        ...
        witness_weight = 1 + 64 + (not chainparams['elements'])
    else:
        assert False
...
def test_withdraw_bech32m(node_factory, bitcoind):
    ...
    # A valid BIP350 encoding can still exceed Blake2b's 34-byte output limit.
    oversized = "bcrt1pw508d6qejxtdg4y5r3zarvary0c5xw7kw508d6qejxtdg4y5r3zarvary0c5xw7k0ylj56"
    with pytest.raises(RpcError, match='bad-txns-vout-script-toolarge'):
        l1.rpc.withdraw(oversized, 10**3)
...
def test_onchain_missing_no_p2tr_migrate(node_factory, bitcoind):
    ...
    bitcoind.env['BLAKE2B_ACTIVATION_HEIGHT'] = str(len(blocks) + 1)
    bitcoind.start()
