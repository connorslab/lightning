...
def _pyln_has_inline_plugin():
    """Did pyln-testing gain get_node(inline_plugin=...) yet?

    It arrived after this branch forked, and get_node() takes **kwargs, so on a
    branch without it the argument is silently swallowed and the plugin never
    loads.  Detect it rather than let the test fail somewhere unrelated.
    """
    import inspect
    from pyln.testing.utils import NodeFactory
    return 'inline_plugin' in inspect.signature(NodeFactory.get_node).parameters


@unittest.skipIf(not _pyln_has_inline_plugin(),
                 "pyln-testing here has no get_node(inline_plugin=): "
                 "the helper plugin cannot be loaded")
def test_blinded_forward_policy(node_factory):
    ...
    assert "onionreply" in err.value.error["data"]
    # Must not SIGFPE on u32 wrap, and must not fail at onion decode.
    assert not l2.daemon.is_in_log(r"Failing HTLC because of an invalid payload")
    l3.daemon.wait_for_log(r"final incorrect amount")
